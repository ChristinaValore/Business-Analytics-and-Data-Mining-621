...
We can see the full view of the influential numbers by adding to a dataframe and viewing the head. We have 89 influential outliers. To take it one step further, we can find out what is the most influential outlier. 
...
The variables we are using: TEAM_BATTING_H, TEAM_BATTING_BB, TEAM_PITCHING_H, TEAM_PITCHING_BB luckily do not have any missing values. Should we decide to use additional variables, we will revisit this section. 
...
Our thought is that the team that is on base more, will have a higher chance of winning and thus more wins. 

```{r}

train$TEAM_BATTING_OB <- train$TEAM_BATTING_H + train$TEAM_BATTING_BB # create variable team batting on base

train$TEAM_PITCHING_A <- train$TEAM_PITCHING_H + train$TEAM_PITCHING_BB # create variable pitching allows

train$BASE_DIFF<- train$TEAM_BATTING_OB - train$TEAM_PITCHING_A #create variable base diff
  
```


```{r}
plot(train$BASE_DIFF, train$TARGET_WINS, xlab = "Homeruns", ylab = "Wins", title("Wins vs. homeruns"))
```
