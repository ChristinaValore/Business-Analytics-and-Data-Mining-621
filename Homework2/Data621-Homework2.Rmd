...
roc_curve = function(data, trueClass, probabilityColumn) {
  ...
  #attach ggplot roc curve and auc value to a list
...
confusionMatrix(reference = as.factor(data$class), as.factor(data$scored.class))
caret::sensitivity(reference = as.factor(data$class), as.factor(data$scored.class))
caret::specificity(reference = as.factor(data$class), as.factor(data$scored.class))
```
The results are the same as the functions created above.
